...
test("candidatePacks substring quick-reject keeps packs whose keyword appears", () => {
  ...
  assert.deepEqual(
    reg.candidatePacks("git reset --hard").map((p) => p.id),
    ["core.git"],
  );
  assert.deepEqual(
    reg.candidatePacks("docker system prune").map((p) => p.id),
    ["containers.docker"],
  );
  assert.deepEqual(
    reg.candidatePacks("ls -la").map((p) => p.id),
    [],
  );
});
...
test("candidatePacks uses plain substring (DCG might_match), so .gitignore still hits 'git'", () => {
  ...
  assert.deepEqual(
    reg.candidatePacks("cat .gitignore").map((p) => p.id),
    ["core.git"],
  );
});
...
test("empty keyword list => pack always a candidate (no keywords = always check)", () => {
  ...
  assert.deepEqual(
    reg.candidatePacks("anything at all").map((p) => p.id),
    ["safe.always"],
  );
  assert.deepEqual(
    reg.candidatePacks("").map((p) => p.id),
    ["safe.always"],
  );
});
...
test("multiword (whitespace) keyword matches with collapsed inter-token whitespace", () => {
  ...
  assert.deepEqual(
    reg.candidatePacks("docker system prune").map((q) => q.id),
    ["x.multi"],
  );
  assert.deepEqual(
    reg.candidatePacks("docker   system prune").map((q) => q.id),
  ...
  assert.deepEqual(
    reg.candidatePacks("dockersystem").map((q) => q.id),
    [],
  );
});
